#include <stdio.h> /* Лабораторная работа №4. Сделал Пилипенко Александр, 1 ПМ, 4гр. АДМО */
int main() {
    FILE *fd = fopen("..\\pilipenko_time_lab4.txt", "r"); // .. - это относительный путь для файла для первой задачи
    if (!fd) {
...
/*
#include <stdio.h> // Лабораторная работа №4. Второй способ для 1 задания. Сделал Пилипенко Александр, 1 ПМ, 4гр. АДМО
int main() {
    int max_count = 0, count = 0, enterTime[24][60] = {0}, exitTime[24][60] = {0}; // max_count - макс. кол-во посетителей в единицу времени за весь день, count - кол-во посетителей в данный момент времени, enterTime - хранит время входа посетителей, exitTime - хранит время выхода посетителей
